...
- [x] **T.10 Image-tint blend (core)** — soften T.8's full-duotone retoning into a runtime-tunable lerp between each themable image's baked palette and the duotone target, default 50 % (FR-15.6). Add a `volatile uint8_t s_image_tint_pct` (0..100) and a `blend565(orig, target, pct)` helper next to `pack565()` in `src/render/theme.cpp`; replace the `dst[k] = lut[e.lum[k]]` write in `rebuild_runtime_palettes()` with the blend. Short-circuit at 0 (copy `e.palette[k]`) and 100 (today's behaviour, bit-identical). Expose `theme::set_image_tint_pct(uint8_t)` / `theme::image_tint_pct()` reusing the existing per-image double-buffer + atomic active-byte flip (FR-15.4 — no torn frames). Apollo unaffected at every value (`bg_ramp() == nullptr` early-return is preserved). For T.10 the value is hardcoded to 50 in the file scope. **Exit:** non-Apollo themes render BMPs as the original photo with a soft theme-coloured cast, not solid duotone; Apollo pixel-identical to today; theme-switch wall time still ≤ 5 ms.
- [x] **T.11 Image-tint MQTT + prefs (FR-15.6, FR-18.2 v2)** — wire user control. Extend `observatory/theme` handler in `mqtt_link.cpp` to accept an optional `tint: 0..100` field alongside `id` (either or both required; out-of-range or non-int → reject the whole payload per FR-1.3). Add `image_tint_pct` to `prefs::Prefs` with default 50, bump `prefs::kSchemaVersion` to 2; the existing P.2 forward-compat passthrough handles a v1 file on disk (missing key → default applies). New `prefs::set_image_tint_pct(uint8_t)` mirrors `set_theme()`: clamps, stores under `s_mutex`, marks dirty, arms the FR-18.4 writeback, calls `theme::set_image_tint_pct()`. Status heartbeat (`observatory/status`) gains `image_tint_pct: <0..100>` (FR-15.7). HA discovery in `homeassistant/setup_mqtt.py` adds `number.observatory_image_tint` (min 0, max 100, step 5) publishing `{"tint": <val>}` to `observatory/theme`. **Exit:** `mosquitto_pub -t observatory/theme -m '{"tint":0}'` renders every themable BMP in its original palette under any theme; `{"tint":100}` matches T.8's pre-T.10 behaviour bit-for-bit; reboot restores the last value within ~35 s; heartbeat reports the current value within one publish cycle.
- [x] **T.12 Image-tint docs** — reconcile shipped behaviour with the spec. Update `docs/THEME.md` §6.2 to describe the lerp formula (was: "no per-pixel work"); add §6.9 "Tint strength" with the 0..100 % control surface, default 50, MQTT/HA hookup, persistence, and the two short-circuit edges. Update `docs/MQTT_TOPICS.md` `observatory/theme` table with the optional `tint` field and add an `{"id":"...","tint":N}` combined example; add `image_tint_pct` row to the `observatory/status` heartbeat table. Tick T.10 / T.11 / T.12. **Exit:** `grep -n 'image_tint_pct\|tint' docs/REQUIREMENTS.md docs/PLAN.md docs/THEME.md docs/MQTT_TOPICS.md` shows the contract in all four docs and matches the wire protocol the firmware actually accepts.
