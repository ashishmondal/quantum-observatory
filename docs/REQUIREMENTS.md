...
- **FR-9.7** The dedicated `clock` scene (FR-9.4) SHALL render the giant HH:MM as an animated 7-segment LCD face composed of these layers, in draw order:
  1. **Per-theme animated background** behind the digits (FR-15.10) — every theme provides its own signature motion (e.g. CRT raster scan, drifting nebula, vector grid). The giant readout self-cleans against it via the layers below; no opaque slot fill is permitted.
  2. **LCD ghost layer** — the union of all segments that can ever light in 12-hour mode (`"18:88"`) in `Ink::GHOST` with a black halo, drawn first so the unlit-segment shadow reads as etched against any background.
  3. **Per-slot live digits** for H1 H2 M1 M2 in `Ink::GIANT_DIGITS` over the ghost. The minute-units (M2), ten-minutes (M1), and hour-units (H2) slots SHALL each animate with a **two-phase rolodex/odometer roll** when their value changes: a constant-speed digit scramble (~30 ms/step) followed by a fixed ease-out cubic settle on the new value. Roll changes SHALL **cascade** from the highest-order changed slot rightward, with per-slot durations of 300 / 600 / 900 ms so the lower-order slots keep ticking at a uniform digit cadence while the upper slot is still easing. The H1 slot is blank or `1` only and never rolls. A boot-time invalid→valid transition (or any `--:--` dropout) SHALL silently latch the new value without triggering a cascade.
  4. **Colon (`:`) slot** — pulses at 1 Hz. The slot background SHALL remain transparent (no `fillRect`, no halo behind the colon glyph) so the per-theme background animation is visible between and around the two colon dots. A dim ghost `:` is drawn first; the live colon overlays at full theme value for the first ~100 ms of each wall-clock second, then linearly fades to a ~10 % floor over the remaining ~900 ms (the floor keeps the separator legible throughout the second).
  5. **Date strip** below a thin theme-dim divider (suppressed under themes whose background already provides strong horizontal motion behind the digits — currently Apollo's CRT raster and Vectrex's perspective grid).

  Each visible digit step in (3) MAY drive one short audible click (FR-10.6 spirit) so the cascade is felt as well as seen; the firmware SHALL gate clicks per (slot, step) so a held frame produces no extra clicks. A diagnostic trigger (IR under `-DCLOCK_ANIM_TEST` and/or MQTT) MAY synthesise MINUTE / TEN_MIN / HOUR cascades on demand without waiting for the wall clock.
...
- **FR-10.4** The buzzer SHALL default to off after boot. *(Superseded by FR-10.8: the prior allowance for a ≤ 50 ms boot self-test chirp and a ≤ 2000 ms "device awake" boot melody is withdrawn — the buzzer is unconditionally silent during the boot phase. The driver still SHALL leave its pin LOW after `begin()` so an immediately-following IR press or theme cue, fired only after boot completes per FR-10.8, does not double-attack the piezo.)*
...
- **FR-10.8 Boot quiet.** The firmware SHALL suppress every buzzer output (FR-10.1 alerts, FR-10.6 IR-press chirps, FR-10.7 theme melodies, the giant-clock digit-roll ticks, and any future cue) for the duration of the boot phase. The boot phase begins at power-on and ends when the FR-13.1 splash overlay clears (i.e. the first successful MQTT connect, which calls `scene_state::set_splash_active(false)`); on a device that never reaches MQTT, the splash never clears and the buzzer remains silent. Any `buzzer::play()` / `chirp()` / `tick_click()` call issued during the boot phase SHALL be dropped at the driver boundary — not queued, not deferred — so a rising-edge cue fired during boot (e.g. a theme `set()` triggered by `prefs::current()` restore) does not produce a delayed beep after the silence lifts. An in-flight melody at the moment quiet is engaged SHALL be stopped immediately (`noTone()`).

- **FR-10.9 Night quiet.** When `light_sensor::is_night()` is true (i.e. the FR-7.2 night override is active), the firmware SHALL suppress every buzzer output by the same driver-boundary mechanism as FR-10.8 — the device makes no sound in a darkened room regardless of which input gesture or scene event would normally cue one. The suppression SHALL engage on the rising edge of `night_active` (in-flight melody stopped) and SHALL lift on the falling edge with no queued playback. Boot quiet (FR-10.8) and night quiet (FR-10.9) SHALL compose with logical OR: the buzzer is audible only when *neither* override is active.
...
- **FR-14.3** Per-body visibility classification (above-horizon-and-dark / above-horizon-but-daylight / below-horizon) SHALL be derived on-device from the body's bearing + elevation (pushed by HA at low cadence — 15 min is sufficient), the RTC time, and the observer location. The `planets` scene consumes this derivation to decide between the headline framings (`VIS BBBxEE`, `IN <IAU>`, `BELOW`) when the scene's active body matches the pushed `name` on `observatory/planet`. (The body-specific `jupiter_visibility` scene this requirement originally described was generalised into the catalog-driven `planets` scene; the wire-side topic moved from `observatory/jupiter` to `observatory/planet` carrying a `name` field.)
...
- **FR-14.6** Rocket-launch countdown (T-minus to the next scheduled launch) SHALL be derived on-device from the live RTC local epoch and a launch envelope pushed by HA at low cadence (10 min in v1 — see [MQTT_TOPICS.md](MQTT_TOPICS.md) `observatory/launch`). HA performs raw API pass-through against **The Space Devs' Launch Library 2** (`https://ll.thespacedevs.com/2.3.0/launches/upcoming/`); HA runs a two-pass selector — Pass A retains a just-launched entry (LL2 status 3/4/6/7) for 30 min post-t0 so the outcome is visible, Pass B otherwise picks the soonest upcoming entry — converts every time-bearing field from UTC to **HA-local epoch** (matching the frame the RTC stores per the Phase 3.6.3 decision), and emits `{t0_local_epoch, t0_estimate, t0_window_close_local_epoch?, provider, vehicle, mission, pad_code, org?, pad_country?, description?, result}`. Provider abbreviation comes from LL2's `launch_service_provider.abbrev`; pad abbreviation from an operator-editable Python dict; `org` is the full provider name; `pad_country` is the country (or `"STATE, USA"` for US sites) parsed from the last comma-segment of `pad.location.name`; `description` is the NFKD-folded ASCII-clipped mission blurb. None of these are baked into firmware flash. Carrying local-epoch on the wire eliminates all tz math on the firmware side — the countdown reduces to `t0_local_epoch - tod::now().local_epoch`, with zero compile-time tz constants in the render path. The `launch_countdown` scene consumes this snapshot to render a six-cell `T-HH:MM:SS` odometer (sliding-digit animation), a `T+HH:MM:SS` count-up for up to 35 min post-t0, a `~T` prefix in `STATUS_WARN` ink whenever `t0_estimate=true`, a typewriter info row cycling mission identity / org / vehicle / country / liftoff time / NET-or-CONFIRMED / window / post-t0 outcome, and a bottom marquee scrolling `description`. Snapshot freshness window SHALL be **4 h** (`launch_state::kFreshMs`); past freshness the odometer freezes on dashes and the marquee shows `AWAITING SCHEDULE`. The scene SHALL emit one audible cue per integer second in the final 10 s window and one "ignition" sting at `T-0`, both gated by the FR-19.4 `button_sound` pref AND the FR-10.9 night-quiet gate.

- **FR-14.6a** *Imminent auto-switch.* A Core 0 1 Hz edge detector ([src/state/launch_imminent.cpp](../src/state/launch_imminent.cpp)) SHALL preempt the active scene with `LAUNCH_COUNTDOWN` at **priority 5 sticky** (the FR-2.1 ceiling) on the rising edge of `0 ≤ t_minus ≤ 300 s`, and play a `C6 → E6 → G6 → C7` heads-up fanfare distinct from the ISS "ta-da" and the T-0 ignition sting. The grab SHALL be suppressed entirely while `light_sensor::is_night()` is true (sleeping-operator courtesy) — the FR-10.9 buzzer mute and the FR-7.2 NIGHT compositor overlay are the second and third lines of defence. An operator IR/MQTT request with `user_intent=true` defeats the sticky lock at any time. The falling edge clears the sticky only when `LAUNCH_COUNTDOWN` is still the active scene (preserving an operator's manual navigation away during the window).
...
- **FR-15.9** Every theme SHALL bind exactly four font roles forming an ascending size ladder: `MICRO` (1–3 character indicators only) < `BODY` (readable lines) < `HEADER` (bracketed labels / button prompts) < `CLOCK` (giant HH:MM digits). `MICRO` SHALL be Tiny3x3 and `CLOCK` SHALL be Digital-7 mono 14pt across every theme — only `BODY` and `HEADER` vary by theme. Scenes SHALL request a font by role via `theme::font(FontRole)` and SHALL NOT assume any specific physical font is bound to a role.
- **FR-15.10** Every theme SHALL provide a **per-theme animated background** for the giant `clock` scene (FR-9.4 / FR-9.7), surfaced through the shared `BgType::THEME_CLOCK` slot in the background dispatcher and dispatched to the active theme via `Theme::init_clock_bg()` / `Theme::render_clock_bg()`. The animator SHALL be visually distinct per theme (e.g. CRT raster scan for Apollo, drifting nebula for Nostromo, perspective grid for Vectrex) and SHALL be re-initialised on every theme switch so the new theme's animator starts from a clean state at the next-frame boundary (FR-15.4).
...
| `exoplanet_count` | (none, procedural planet) | [EXO] header + animated procedural planet (seeded from `nearest_name`) + typewriter `TOT N` / `±N WK` / `NR x.xLY` | astrophysics Tier-2 backlog scene (FUTURE_SCENES.md); HA pyscript daily poll of NASA Exoplanet Archive TAP → `observatory/exoplanet`; on-device freshness ceiling 48 h; opts OUT of FR-9.2 clock chrome (parked planet at cx=50,r=12 occupies the top-right HH:MM region — same trade-off as GiantClockScene); reusable `planet::ProceduralPlanet` renderer (FNV-1a hash → 8 archetypes × hue × surface features, Lambert-shaded sphere, ≤ 16 px radius) |
...
| **W7.6 — Launch countdown** | `launch_countdown` scene, 10 min HA poll of Launch Library 2, on-device T-minus, 5-min imminent auto-switch | Phase L |
