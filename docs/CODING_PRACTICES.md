...
- **Bit depth caps fade resolution.** Per-channel weight math (AA, trail fade) only produces visible gradients if base color × weight stays ≥ 1 LSB at the panel's bit depth. We use `PANEL_BIT_DEPTH = 6` (64 levels) and base colors ≥ ~25% so dim weights don't round to 0. (added in phase 2.3)
