...
private:
  ...
  // Point ImagePaletteBg at the first artist-supplied bitmap, if any.
  // Registry is generated by tools/bmp_to_header.py from assets/*.bmp;
  // when the assets folder is empty, kImageRegistryCount is 0 and we
  // leave the renderer pointed at nullptr (renders black, by design).
  void init_image_default() {
    if (kImageRegistryCount > 0) {
      const ImageEntry& e = kImageRegistry[0];
